...
class MecanicasManzana:
    ...
    def colocar_manzana_random(self, segmento_serpiente_x, segmento_serpiente_y, direccion_actual, segmentos_serpiente):
        ...
        posiciones_validas = []
        # Iterar sobre todos los posibles puntos de la cuadrícula en el área objetivo
        for xs in x_pasos:
            for ys in y_pasos:
                if (xs, ys) not in coords_a_evitar:
                    # Convierte los pasos de cuadrícula a coordenadas de píxeles
...
class JuegoSerpiente:
    ...
    def dibujar_puntuacion(self, superficie):
        ...
        texto = fuente.render(f"Puntuación: {self.puntuacion}", True, NEGRO)
    ...
    def mover_serpiente(self):
        ...
        # 1. Aplicar el efecto de teletransporte (wrapping) - ¡Nueva lógica!
        # Usa el operador módulo (%) para que la posición se "envuelva" al cruzar el límite.
